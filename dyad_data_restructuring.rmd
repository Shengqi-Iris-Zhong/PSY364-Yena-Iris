...
prem_diary <- final_diary %>%
  ...
  filter(gender_sum == 0) %>%
  dplyr::select(dyadID = dyadID.x, obsid, r_years, childnum, gender, gender_chr,partID, day, day_of_study, partnum,  telework, income, grbs_ss, fair_chores, qmi_ss)
```
...
```{r}
write.csv(prem_diary,"prem_diary.csv")
```
...
tempA <- prem_diary %>%  
  ...
  gather(variable, value, gender_chr:qmi_ss) %>%
